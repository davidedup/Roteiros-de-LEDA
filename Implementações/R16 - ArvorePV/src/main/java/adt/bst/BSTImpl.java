...
public class BSTImpl<T extends Comparable<T>> implements BST<T> {
	...
	// amintas ta fazendo
	public int diameter(BSTNode<T> node) {
		int leftHeight = 0;
		int rightHeight = 0;
		...
		int string = leftHeight + rightHeight;
		if (string > diameter((BSTNode<T>) node.getLeft()) && string > diameter((BSTNode<T>) node.getRight())) {
			return string;
		}
		return Math.max(diameter((BSTNode<T>) node.getLeft()), diameter((BSTNode<T>) node.getRight()));
	}
